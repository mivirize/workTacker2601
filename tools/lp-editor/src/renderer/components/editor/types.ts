...
export interface RepeatItemFieldData {
  id: string
  type: 'text' | 'richtext' | 'image' | 'link' | 'background-image' | 'number' | 'icon' | 'counter'
  label?: string
  value: string | null
...
export interface RepeatItemData {
  index: number
  fields: Record<string, RepeatItemFieldData>
}

export interface RepeatBlockData {
  id: string
  min: number
  max: number
  items: RepeatItemData[]
  templateHtml: string
}

export interface ColorFieldData {
  variable: string
  value: string
  label?: string
  description?: string
...
// Predefined SVG icons for icon fields
